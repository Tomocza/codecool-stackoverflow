...
public class AnswersDAOJdbc implements AnswersDAO {
  ...
  public List<AnswerModel> getAnswersByQuestionId(int questionId, int userId) {
    ...
    String sql =
            ...
            " rating, coalesce(vc.vote_count, 0) as has_voted from answers a left join answer_votes av on a.id = av" +
            ".answer_id left join (select av.answer_id, av.value as vote_count from answer_votes av where av.user_id " +
            "= ? group by av.answer_id, av.value) vc on a.id = vc.answer_id where a.question_id = ? group by a.id, " +
            "has_voted order by rating desc ";
  ...
  public Optional<AnswerModel> getAnswerById(int answerId) {
    String sql =
            ...
            " rating, coalesce(vc.vote_count, 0) as has_voted from answers a left join answer_votes av on a.id = av" +
            ".answer_id left join (select av.answer_id, av.value as vote_count from answer_votes av where av.user_id " +
            "= ?) vc on a.id = av.answer_id where a.id = ? group by a.id, has_voted";
