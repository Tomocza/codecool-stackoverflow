...
public class QuestionsDaoJdbc implements QuestionsDAO {
  ...
  public List<QuestionModel> getAllQuestions() {
    ...
    String sql =
            "select q.id, q.title, q.body, q.user_id, q.created_at, q.modified_at, count(distinct a.id) as " +
            "answer_count, coalesce(qv.rating, 0) as rating, coalesce(vc.vote_count, 0) as has_voted from questions q" +
            " left join answers a on q.id = a.question_id left join (select question_id, sum(value) as rating from " +
            "question_votes group by question_id) qv on q.id = qv.question_id left join (select question_id, value as" +
            " vote_count from question_votes where user_id = ? group by question_id, value) vc on q.id = vc" +
            ".question_id group by q.id, qv.rating, vc.vote_count order by q.id desc ";
  ...
  public Optional<QuestionModel> getQuestionById(int id, int currUserId) {
    String sql =
            "select q.id, q.title, q.body, q.user_id, q.created_at, q.modified_at, count(distinct a.id) as " +
            "answer_count, qv.rating, coalesce(vc.vote_count, 0) as has_voted from questions q left join answers a on" +
            " q.id = a.question_id left join (select question_id, value as vote_count from question_votes  where " +
            "question_id = ? and user_id = ?) vc on q.id = vc.question_id, (select sum(value) as rating from " +
            "question_votes where question_id = ?) qv where q.id = ? group by q.id, qv.rating, vc.vote_count";
  ...
  public List<QuestionModel> getQuestionByName(String name) { // not finished, need the query
    ...
    String sql =
            ...
            "answer_count, coalesce(qv.rating, 0) as rating, coalesce(vc.vote_count, 0) as has_voted from questions q" +
            " left join answers a on q.id = a.question_id left join (select question_id, sum(value) as rating from " +
            "question_votes group by question_id) qv on q.id = qv.question_id left join (select question_id, count(*)" +
            " as vote_count from question_votes where user_id = ? group by question_id) vc on q.id = vc.question_id " +
            "where q.title ilike '%' || ? || '%' group by q.id, qv.rating, vc.vote_count";
    try (Connection connection = connector.getConnection();
         PreparedStatement statement = connection.prepareStatement(sql)) {
